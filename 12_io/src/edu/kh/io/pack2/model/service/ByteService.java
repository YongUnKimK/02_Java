...
/* 
  ...
  *  모든 것을 입/출력할 수 있음
  *  (단, 1byte라서 통로가 좁다보니 속도가 좀 느림, char
  *  형 문자는 깨질 우려가 있음)
  * 
  * 
  * */
...
//			}
			...
			System.out.println("수행 시간 : " + (endTime - startTime) + "ns"); // 71500ns
			//이시점 write() 수행 후 시간
	...
	public void bufferedFileByteOutput() {
		 ...
		 * -> 통로가 좁아서 데이터 입출력이 느림
		 * 
		 * -> 보조스트림 이용시 버퍼에 입출력할 내용을 모아서 한 번에 입출력!
	...
	public void fileByteInput() {
		...
		try {
			...
			while (true) {
				...
				sb.append( (char)value ); // char로 강제 형변환
	 ...
	 *  복사 완료 : /io_test/20240926/노래가사.txt
	 *  
	...
	public void fileCopy() {
		...
		try {
			...
			// 해당 경로에 파일이 존재하면
			// target이 가리키는 파일을 입력 받을 수 있또록
			// 입력용 스트림 생성
			
			fis = new FileInputStream(target); // 기반 입력용 스트림 생성
			bis = new BufferedInputStream(fis); // 보조 입력용 스트림 생성
