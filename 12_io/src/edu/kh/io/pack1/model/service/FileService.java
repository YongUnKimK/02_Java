...
public class FileService {
	...
	/* File 클래스
		 *I : input -> 입력
			...
			모든 스트림은 단방향이며 각각의 장치마다 연결할 수 있는 스크림 존재
			하나의 스트림으로 입출력을 동시에 수행할 수 없으므로 동시수행하기위해서는 2개의 스트림 필요함.
		 * - java.io 패키지
		 *
	...
	public void method1() {
		...
		// 경로 제일 앞에 "/"의 의미 : 최상위 폴더를 의미( == root, 절대 경로의 기준점)
	...
	public void method2() {
		...
		// -> 역슬래시 자체를 문자로 사용하고 싶을때는 2개를 사용 //
		
		//문자열에서 "/" 표기하는 방법 : "//"
		
		File file = new File("\\io_test\\20240926\\파일생성.txt");
		
		if( !file.exists()) { //존재하지 않으면 파일을 생성한다
			try {
	...
	public void method3() {
		...
		for (File f : files) {
			...
			// 파일 유형
			String type = null;
			if (f.isFile()) { // boolean  isFile()         : 현재 File 객체가 관리하는게 파일이면 true
				type = "파일";
			} else type = "폴더";
	 ...
	 * 즉, 파일이나 폴더에 실제 데이터를 읽거나 쓸 떄 스트링을 이용함.
