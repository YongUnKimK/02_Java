...
public class ToyFactory extends Toy {
	...
	public void ToyFactory() {
		...
		// 이후 addMaterials의 전달인자를 List<Integer> 로 바꿈
	 ...
	 * 
	 * 가변인자 작성법 : 자료형 ... 변수명
	 * -> 매개변수의 수가 정확히 몇개인지 특정할 수 없을떄 사용!
	 * @param newMaterials
	 * @return
	 */
	
	
	public Set<String> addMaterials(Integer...newMaterials /* List<Integer> materials */) {
		...
		Set<String> addedMaterials = new HashSet<String>();
			...
			for(Integer materialKey : newMaterials ) {
				...
				// Map에서 해당 번호(key)에 대응하는 재료를 가져와 
				// addedMeterials에 추가
				String materialValue = materialMap.get(materialKey);
				
				// map에 없는 key를 입력하여 value를  얻어왔을떄는 null
				
				if(materialValue != null) { // 재료 목록에 있을 떄
...
public void displayMenu() {
	...
	public void createNewToy() throws Exception {
		...
		Set<String> materials = new HashSet<String>(); // 재료 저장할 Set객체 생성
		
		System.out.println("사용 가능한 재료 목록");
		
		// System.out.println(materialMap.entrySet());
		// [1 = 면직물, 2 = 플라스틱, 3=유리, 4=고무] <- entrySet()
		...
		while (true) {
			...
			try {
				...
				int materialKey = Integer.parseInt(input); // <- 문자열을 int형으로 바꿔줌 "1" ->  1
				...
				} else { // 재료명이 null == 없는 재료 고유번호를 입력
					System.out.println("잘못된 번호입니다. 다시 입력해주세요.....");
				}
				
			} catch(NumberFormatException e ) { // 재료번호 입력 시 숫자나 q가 아닌 경우 예외처리
	...
	public void deleteToy() throws Exception{
		...
		for (Toy toy : toySet) {
			if(toy.getName().equals(toyName)) {
				...
				break; 
				// 1. 효율 향상을 위해 break;
				// 2. 순회중인 컬렉션 개체가 수정되어 충돌한다는 예외 발생
				// -> 예외 방지 및 효율을 위해서 작성
			}
		}
	...
	public void addMaterial() throws Exception{
		...
		if(materialMap.containsKey(key)) { // materialMap에 이미 존재하는 Key 인지 확인
			...
			if(input.equalsIgnoreCase("Y")) {
				...
				System.out.println("재료추가가 취소되엇습니다.");
			}
	...
	public void displayToysByAge() {
		System.out.println("<연령별 사용 가능한 장난감 순으로 정렬>");
