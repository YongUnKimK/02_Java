...
public class ArrayExample1 { // 예제 작성용 클래스
	...
	public void ex3() {
		...
		// double[] 자료형 참조변수 height stack 영역에 생성하고 
	...
	public void ex4() {
		...
		for(int i = 0; i < score.length; i++) {
			...
			score[i] = sc.nextInt(); // score[0] = 100 .. 
			sum += score[i];
		}
		// =========================================
	...
	public void ex5() {
		...
		// Arrays.toString(배열명) : 모든 요소 값 출력쓰
		System.out.println(Arrays.toString(arr) );
		...
		System.out.println(Arrays.toString(arr3)); // 기본값 false
	...
	public void ex6() {
		...
		// 점심 메뉴 뽑기 프로그램
		String[] arr = {"김밥", "서브웨이", "햄버거", "백반", "도시락", "짜장면"};
	...
	public void ex7() {
		...
		// -> ! (not연산자 : 부정 논리 연산자) 붙여줌으로써 true로 변경하여		
		// {} 안에 있는 코드를 수행하게 함
	...
	public void ex8() {
		...
		for (int i = 0 ; i < arr.length; i++) {
			if( arr[i].equals(input) ) { 
				System.out.println(i + " 번 인덱스에 존재합니다");
				flag = true;
			}
		} if ( !flag ) {System.out.println("존재하지 않음");}
	...
	public void ex9() {
		...
		*
		...
		// 1. 문자열을 입력받아 한글자씩 잘라내어 char 배열에 순서대로 저~장
		...
		// sc.next() -> String 형
		// String.charAt(0) -> String의 0번쨰 인덱스 문자 하나를 추출해서 char 형태로 반환
		// "h" -> 'h' ..
		
		int count = 0; // 같은 글자 개수를 세기 위한 변~수
		for (int i = 0; i < arr.length; i++) {
