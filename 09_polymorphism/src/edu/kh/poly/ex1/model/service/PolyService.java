...
public class PolyService {
	...
	public void ex1() {
		...
		// Spark 객체를 참조하는 변수(car3) 타입이 Car
		// Spark 객체가 Car 객체로 변화함
		// - > 업캐스팅
		
		// 업캐스팅 작성 방법
		
		// 1) 자식 객체가 부모 객체로 변하였기 때문에(업캐스팅)
		...
		tesla.setFuel("전기");
		tesla.setWheel(4);
		tesla.setBatteryCapacity(1100000);
		...
		// setBatteryCapacity(int) is undefined for the type Car
		...
		// 다형성(업캐스팅) : 부모타입 참조변수 arr[i]로 자식객체를 참조할 수 있따.
		
	}
	// 전달받은 Car 또는 자식객체(Tesla, Spark)의 엔진, 연료, 바퀴개수를 출력하는 메서드
	...
	public void ex2() {
		...
		System.out.println( arr[1] instanceof Spark); // false (Spark와 Tesla 관계없
	...
	public void ex3() {
		...
		// 다운캐스팅 : 부모타입 참조변수가 자식객체를 참조하는 기술(업캐스팅)
		// 적용된 상태에서만 진행할 수 있다.
		// 부모타입을 자식타입으로 "강제 형변환"해서
		// 자식객체의 본래 필드, 메서드를 사용 가능하게 한다.
		
		Car c1 = new Tesla("전기모터", "전기", 4, 50000); // 업캐스팅 ( Tesla 자식 -> Car 부모)
	...
	public void ex4() {
		...
		Car c1 = new Tesla(); // Tesla 객체를 만들어 Car타입(부모타입) 참조변수 c1에 담음
		
		//Spark s1 = (Spark)c1; // c1을 관계없는 Spark로 다운캐스팅하려고함
		// 다운캐스팅 잘못해도 컴파일 에러 발생 x
		// -> 실행시 발생(런타임 에러)
