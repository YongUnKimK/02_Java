...
import edu.kh.oop.method.model.vo.Member.Member;
...
public class MemberService {
	...
	public String login() {
		...
		// 회원 가입 정보(memberInfo가 참조하는 Member객체)에 저장된
		// 아이디와 비밀번호가 입력한 아이디, 비밀번호와 일치하면 
		// "로그인 성공" , 같지 않으면 " 아이디 또는 비밀번호 불일치" 리턴
		
		if(memberId.equals(memberInfo.getMemberId()) && memberPw.equals(memberInfo.getMemberPw())) {
			
			//로그인하려고 입력받은 memberId와
			// memberInfo필드에서 참조중인 Member객체의 memberId가 같은지 확인
			// 로그인하려고 입력받은 memberPw와
			// memberInfo필드에서 참조중인 Member 객체의 memberPw가 같은지 확인
	...
	public void read() {
		//로그인을 했나 안했나 따져보기
		//로그인 했으면 현재 로그인한 사람의 정보를 저장하고 있는 객체에서 정보 꺼내오기 (이름,아이디, 나이)
		//로그인안했으면? 로그인후이용해라
		System.out.println("================회원정보================");
		if ( memberInfo == null) { System.out.println("회원가입 다시 진행하시오");  } 
		else {  System.out.printf("이름 : %s\n"
				+ "아이디 :  %s\n"
				+ "나이 : %s\n ", loginMember.getMemberId(), loginMember.getMemberName(), loginMember.getMemberAge()  );
