...
//		1) 서버의 포트번호 정함
		...
		// 입/출력용 참조변수 선언
		InputStream is = null; // 바이트기반  스트림 최상위
		...
		try {
//			2) 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // 8500번 포트로 만들어진 소켓
			
//			3) 클라이언트 쪽에서 접속 요청이 오길 기다림
//			4) 접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
			System.out.println("[Server]");
			System.out.println("클라이언트의 요청을 기다리고 있스빈다....");
...
//			7) 스트림을 통해 읽고 쓰기
			...
			//  now Date() 로 객체 생성 시 현재 시간이 저장되어있다.
